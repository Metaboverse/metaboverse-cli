...
def test():
    ...
    graph = nx.DiGraph()
    graph.add_node('OAA')
    graph.nodes()['OAA']['values'] = [None, None, None]

    # rxn 1
    graph.add_node('rxn1')
    graph.add_node('MAL')
    graph.nodes()['MAL']['values'] = [1, 2, 3]
    graph.add_node('NAD+')
    graph.nodes()['NAD+']['values'] = [None, None, None]
    graph.add_node('NADH')
    graph.nodes()['NADH']['values'] = [None, None, None]
    graph.add_edges_from([('rxn1', 'MAL')])
    graph.add_edges_from([('rxn1', 'NAD+')])
    graph.add_edges_from([('rxn1', 'NADH')])
    graph.add_edges_from([('rxn1', 'OAA')])
    graph.add_edges_from([('MAL', 'rxn1')])
    graph.add_edges_from([('NAD+', 'rxn1')])
    graph.add_edges_from([('NADH', 'rxn1')])
    graph.add_edges_from([('OAA', 'rxn1')])

    # rxn 2
    graph.add_node('rxn2')
    graph.add_node('PYR')
    graph.nodes()['PYR']['values'] = [3, 2, 1]
    graph.add_edges_from([('OAA', 'rxn2')])
    graph.add_edges_from([('rxn2', 'PYR')])

    reaction_dictionary={
        'rxn1': {
            'reactants': ['MAL', 'NAD+'],
            'products': ['OAA', 'NADH'],
            'modifiers': [['faker', 'catalyst']]
        },
        'rxn2': {
            'reactants': ['OAA'],
            'products': ['PYR'],
    ...
    real_reactants, effective_reactants, real_products, \
    effective_products, real_modifiers = get_reaction_values(
        reaction=reaction_dictionary['rxn1'],
        collapse_with_modifiers=collapse_with_modifiers)

    real_reactants_B, effective_reactants_B, real_products_B, \
    effective_products_B, real_modifiers_B = get_reaction_values(
        reaction=reaction_dictionary['rxn2'],
        collapse_with_modifiers=collapse_with_modifiers)
    ...
    real_reactants_exist
    effective_reactants_exist
    real_products_exist
    effective_products_exist
